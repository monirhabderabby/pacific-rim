...
const DashboardSidebarItem = ({ item }: Props) => {
  ...
  return (
    <div className="relative pl-[38px]">
      <Link className={cn(`flex items-center gap-2 px-6 py-4 rounded-l-full text-lg font-medium leading-[21px] text-[#121D42] ${isActive && "bg-[#E6EEF6]"}`)} href={item.href}>{item?.icon} {item?.name}</Link>
